import { createActionItem, type ActionItem } from '@workspace/actions';
import { createCaseAssignment, type CaseAssignment } from '@workspace/assignments';
import { createFollowUpCase, type CasePriority, type FollowUpCase } from '@workspace/cases';
...
export interface CreateOwnCasePlan {
  ...
  readonly primaryResponsibility: CaseAssignment;
...
export function planCreateOwnCase(
  ...
  const primaryResponsibility = createCaseAssignment({
  ...
  const initialAction = createActionItem({
    ...
    caseId: context.identity.caseId,
    organizationId: command.organizationId,
    sourceAssignmentId: primaryResponsibility.id,
    responsible: {
      kind: 'membership',
      membershipId: command.actorMembershipId,
    },
