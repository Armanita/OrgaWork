...
export interface WorkerConnectivityDependencies {
...
export interface WorkerConnectivityReport {
  readonly service: 'orgawork-worker';
...
const defaultDependencies: WorkerConnectivityDependencies = {
...
export async function verifyWorkerConnectivity(
  dependencies: WorkerConnectivityDependencies = defaultDependencies,
): Promise<WorkerConnectivityReport> {
  ...
  try {
    ...
    throw new Error('بررسی اتصال پردازشگر پس‌زمینه به سرویس‌های محلی ناموفق بود.');
  }
}
