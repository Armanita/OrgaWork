...
## 95.23. EVD-031 — شاهد قرارداد هویت و سازمان P2.1

- وضعیت: معتبر و ثبت‌شده
- دامنه: `P2.1 — تثبیت قرارداد دامنه هویت و سازمان`
- سند قرارداد: `docs/contracts/P2.1-IDENTITY-ORGANIZATION-CONTRACT.md`
- User و Membership مستقل: تأییدشده
- وضعیت‌های User، Membership، Invitation و Session: تثبیت‌شده
- سیاست گذرواژه و Argon2id: تثبیت‌شده
- نشست Server Side، Cookie و CSRF: تثبیت‌شده
- سازمان جاری، Session Revision و SET LOCAL: تثبیت‌شده
- Deny by Default و Explicit Deny: تثبیت‌شده
- دعوت و تیم: تثبیت‌شده
- مرز P2.2 تا P2.12 و منع ورود زودهنگام به رابط: تثبیت‌شده
- آزمون متمرکز و کنترل کامل: موفق
- Commit فنی: `29b66f4fa4d9827847bab6eb3619498d88566c2c`
- Commit اختتام: `78612b5459343d05a73ea7e12f074793543e9047`
- Tag اختصاصی: ایجاد نشد
- انتقال رسمی: `P2.2 — ایجاد مدل کاربر، سازمان، عضویت و تیم`

## 95.24. EVD-032 — شاهد مدل دامنه هویت و سازمان P2.2

- وضعیت: معتبر و ثبت‌شده
- دامنه: `P2.2 — ایجاد مدل کاربر، سازمان، عضویت و تیم`
- User و ایمیل نرمال‌شده: پیاده‌سازی و آزمون‌شده
- Organization و Membership: پیاده‌سازی و آزمون‌شده
- Team و TeamMembership: پیاده‌سازی و آزمون‌شده
- وضعیت‌های مصوب و Transitionها: آزمون مثبت و منفی موفق
- عضویت فعال و برابری Organization در Team: آزمون‌شده
- استقلال مدل از Persistence و UI: ممیزی‌شده
- Build مستقیم سه Workspace: موفق
- معماری و امنیت مخزن: موفق
- Commit فنی: `e9615610332bda0be5ad588f6ba3a43a1a5c8d3c`
- Commit اختتام: `75bedac73e64bf144c43ad43339abb36684d3c00`
- Tag اختصاصی: ایجاد نشد
- انتقال رسمی: `P2.3 — ایجاد مهاجرت‌ها و سیاست‌های جداسازی سازمانی`

## 95.25. EVD-033 — شاهد Migration و جداسازی سازمانی P2.3

- وضعیت: معتبر و ثبت‌شده
- دامنه: `P2.3 — ایجاد مهاجرت‌ها و سیاست‌های جداسازی سازمانی`
- Migration: `infra/migrations/0004_create-identity-organization-schema.sql`
- پنج جدول User، Organization، Membership، Team و TeamMembership: ایجاد و آزمون‌شده
- وضعیت‌ها و Roleهای مصوب: Constraint شده
- Composite Foreign Key و یکتایی‌های سازمانی: آزمون‌شده
- Triggerهای Transition، Version و Membership فعال: آزمون‌شده
- RLS فعال و اجباری روی چهار جدول سازمانی: آزمون واقعی
- نبود Tenant Context: بدون مشاهده داده
- جداسازی دو Organization: آزمون واقعی
- درج بین‌سازمانی: ردشده
- اجرای دوباره Migration و Fingerprint: موفق
- Commit فنی: `00863aaace2515b69e8b66a20cf54ebf61a3848c`
- Commit اختتام: `6963796e81580527e3a719895c83c03eb5f71fb1`
- Tag اختصاصی: ایجاد نشد
- انتقال رسمی: `P2.4 — پیاده‌سازی ذخیره امن گذرواژه`

## 95.26. تصمیم طراحی پیش‌تأییدشده P2.13

- تصمیم الزام‌آور: `DEC-UI-001`
- مرجع اصلی طراحی: `Studio Admin`
- مرجع دوم اجزا: `TailAdmin Next.js`
- مرجع مهندسی: `Kiranism Dashboard`
- فونت اصلی: `Vazirmatn`
- مراجع منجمد بیرون از مخزن: تأییدشده
- انتقال منطق احراز هویت مجوز چندسازمانی داده و Backend قالب‌ها: ممنوع
- شروع رابط پیش از `P2.13`: ممنوع
- توقف برای تأیید دوباره پس از بسته‌شدن `P2.12`: حذف‌شده
- مالک اجرا: `P2.13`
- آزمون پذیرش: فارسی `RTL` شمسی دسترس‌پذیری عدم Drift و سازگاری معماری
...
`P1` با شاهد `EVD-030` بسته شده است. `P2.1` با `EVD-031`، `P2.2` با `EVD-032` و `P2.3` با `EVD-033` بسته شده‌اند. `P2.4` مرحله جاری است.

`P2.4` تا `P12` فقط در سطح نیاز، مالک و معیار هدف ردیابی شده‌اند، مگر آنکه شاهد مستقل ثبت شده باشد.

ممیزی جامع تاریخچه تا `P2.3` با `56` فایل آزمون، `285` آزمون، Coverage موفق، Build چهار برنامه، زیرساخت واقعی، PostgreSQL و Smoke هماهنگ با نتیجه فنی بدون شکست ثبت شده است.

## 130. محدودیت نسخه نخست ماتریس

مسیرهای کد و آزمون مراحل بسته‌شده تا `P2.3` ثبت شده‌اند. مسیر دقیق قابلیت‌های `P2.4` تا `P12` هنوز فقط در سطح نیاز، مالک و معیار هدف قرار دارد و باید هم‌زمان با اجرای هر بسته افزوده شود.
