...
- زیرمرحله بسته‌شده: `P1.4.9 — افزودن فرمان‌های آغاز، توقف، گزارش و پاک‌سازی زیرساخت`
- زیرمرحله جاری: `P1.4.10 — آزمون اتصال واقعی برنامه‌ها به سرویس‌های محلی`
- شواهد مرحله: `EVD-009`، `EVD-010`، `EVD-011`، `EVD-012`، `EVD-013`، `EVD-014`، `EVD-015`، `EVD-016` و `EVD-017`
- ریسک `RISK-003`: بسته‌شده
- فرض `ASM-001`: در `P1.4.1` رفع شد؛ Docker در `P1.4.3` نصب و پذیرفته شد
- ادعای پیاده‌سازی فعلی: PostgreSQL 16.14، Redis 7.4.10 و MinIO `RELEASE.2025-09-07T16-13-09Z` روی شبکه اختصاصی و Volumeهای پایدار فعال و سالم‌اند؛ Bucket خصوصی `orgawork-files` به‌صورت خودکار و idempotent ایجاد می‌شود و چهار فرمان یکپارچه `infra:start`، `infra:stop`، `infra:report` و `infra:cleanup` برای مدیریت امن زیرساخت پذیرفته شده‌اند
