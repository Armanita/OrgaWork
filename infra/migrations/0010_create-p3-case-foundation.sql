...
CREATE OR REPLACE FUNCTION public.orgawork_validate_p3_case_update()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, public
AS $orgawork_validate_p3_case_update$
BEGIN
  IF (
    NEW.id <> OLD.id
    ...
    RAISE EXCEPTION 'P3 case ownership fields are immutable'
      USING ERRCODE = '23514';
  END IF;

  IF NEW.version <> OLD.version + 1 OR NEW.updated_at < OLD.updated_at THEN
    RAISE EXCEPTION 'P3 case version or timestamp is invalid'
      USING ERRCODE = '23514';
  END IF;
  ...
  IF NEW.status <> OLD.status AND NOT (
    ...
    RAISE EXCEPTION 'P3 case status transition is invalid'
      USING ERRCODE = '23514';
  END IF;

  RETURN NEW;
END
$orgawork_validate_p3_case_update$;

CREATE OR REPLACE FUNCTION public.orgawork_validate_p3_responsibility_update()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, public
AS $orgawork_validate_p3_responsibility_update$
BEGIN
  IF (
    NEW.id <> OLD.id
    OR NEW.organization_id <> OLD.organization_id
    ...
    RAISE EXCEPTION 'P3 responsibility ownership fields are immutable'
      USING ERRCODE = '23514';
  END IF;

  IF NEW.version <> OLD.version + 1 OR NEW.updated_at < OLD.updated_at THEN
    RAISE EXCEPTION 'P3 responsibility version or timestamp is invalid'
      USING ERRCODE = '23514';
  END IF;
  ...
  IF NEW.status <> OLD.status AND NOT (
    ...
    RAISE EXCEPTION 'P3 responsibility status transition is invalid'
      USING ERRCODE = '23514';
  END IF;

  RETURN NEW;
END
$orgawork_validate_p3_responsibility_update$;

CREATE OR REPLACE FUNCTION public.orgawork_validate_p3_action_update()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, public
AS $orgawork_validate_p3_action_update$
BEGIN
  IF (
    NEW.id <> OLD.id
    OR NEW.organization_id <> OLD.organization_id
    ...
    RAISE EXCEPTION 'P3 action ownership fields are immutable'
      USING ERRCODE = '23514';
  END IF;

  IF NEW.version <> OLD.version + 1 OR NEW.updated_at < OLD.updated_at THEN
    RAISE EXCEPTION 'P3 action version or timestamp is invalid'
      USING ERRCODE = '23514';
  END IF;
  ...
  IF NEW.status <> OLD.status AND NOT (
    ...
    RAISE EXCEPTION 'P3 action status transition is invalid'
      USING ERRCODE = '23514';
  END IF;

  RETURN NEW;
END
$orgawork_validate_p3_action_update$;

CREATE OR REPLACE FUNCTION public.orgawork_validate_p3_current_work_update()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, public
AS $orgawork_validate_p3_current_work_update$
BEGIN
  IF (
    NEW.id <> OLD.id
    OR NEW.organization_id <> OLD.organization_id
    ...
    RAISE EXCEPTION 'P3 current-work identity fields are immutable'
      USING ERRCODE = '23514';
  END IF;

  IF OLD.ended_at IS NOT NULL OR NEW.ended_at IS NULL OR NEW.ended_at < OLD.started_at THEN
    RAISE EXCEPTION 'P3 current-work end transition is invalid'
      USING ERRCODE = '23514';
  END IF;

  RETURN NEW;
END
$orgawork_validate_p3_current_work_update$;

CREATE OR REPLACE FUNCTION public.orgawork_validate_p3_idempotency_update()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, public
AS $orgawork_validate_p3_idempotency_update$
BEGIN
  IF (
    ...
    RAISE EXCEPTION 'P3 idempotency identity fields are immutable'
      USING ERRCODE = '23514';
  END IF;

  IF OLD.state <> 'in_progress' OR NEW.state <> 'completed' THEN
    RAISE EXCEPTION 'P3 idempotency state transition is invalid'
      USING ERRCODE = '23514';
  END IF;

  RETURN NEW;
END
$orgawork_validate_p3_idempotency_update$;

CREATE OR REPLACE FUNCTION public.orgawork_validate_p3_case_invariants()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = pg_catalog, public
AS $orgawork_validate_p3_case_invariants$
DECLARE
...
BEGIN
  ...
  IF resolved_case_status = 'open' THEN
    IF active_primary_count <> 1 THEN
      RAISE EXCEPTION 'Open P3 case requires exactly one active primary responsibility'
        USING ERRCODE = '23514';
    END IF;

    IF active_current_work_count <> 1 THEN
      RAISE EXCEPTION 'Open P3 case requires exactly one active current work'
        USING ERRCODE = '23514';
    END IF;
    ...
    IF (
      ...
      RAISE EXCEPTION 'P3 current action must be active'
        USING ERRCODE = '23514';
    END IF;
    ...
    IF (
      ...
      RAISE EXCEPTION 'P3 responsibility-acceptance current work must target a pending primary responsibility'
        USING ERRCODE = '23514';
    END IF;
  ELSE
    IF active_primary_count <> 0 OR active_current_work_count <> 0 THEN
      RAISE EXCEPTION 'Inactive P3 case cannot keep active responsibility or current work'
        USING ERRCODE = '23514';
    END IF;
  END IF;
...
FOR EACH ROW
EXECUTE FUNCTION public.orgawork_validate_p3_case_invariants();
...
REVOKE ALL ON FUNCTION public.orgawork_validate_p3_case_update() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.orgawork_validate_p3_responsibility_update() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.orgawork_validate_p3_action_update() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.orgawork_validate_p3_current_work_update() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.orgawork_validate_p3_idempotency_update() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.orgawork_validate_p3_case_invariants() FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.orgawork_validate_p3_case_update()
  TO orgawork_runtime;
GRANT EXECUTE ON FUNCTION public.orgawork_validate_p3_responsibility_update()
  TO orgawork_runtime;
GRANT EXECUTE ON FUNCTION public.orgawork_validate_p3_action_update()
  TO orgawork_runtime;
GRANT EXECUTE ON FUNCTION public.orgawork_validate_p3_current_work_update()
  TO orgawork_runtime;
GRANT EXECUTE ON FUNCTION public.orgawork_validate_p3_idempotency_update()
  TO orgawork_runtime;
GRANT EXECUTE ON FUNCTION public.orgawork_validate_p3_case_invariants()
  TO orgawork_runtime;
