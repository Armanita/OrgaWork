...
function loadLocalEnvironment(path: string): void {
  if (!existsSync(path)) {
    throw new Error('.env.local برای آزمون واقعی P3.2 موجود نیست.');
  }
  ...
  await access.transaction(async (transaction) => {
    await transaction.query(
      ...
      [
        input.userA,
        input.userB,
        `p32-a-${input.userA}@example.test`,
        `p32-b-${input.userB}@example.test`,
        input.now,
      ],
    );
    ...
    await transaction.query(
      `INSERT INTO public.orgawork_organizations
       ...
       VALUES
         ($1, 'سازمان آزمون P3 الف', $3, $3, 1),
         ($2, 'سازمان آزمون P3 ب', $3, $3, 1)`,
...
async function inspectP3Schema(access: PostgreSqlAccess): Promise<void> {
  ...
  expectCondition(
    ...
    'جداول canonical P3.2 کامل نیستند.',
  );
  ...
  expectCondition(
    ...
    'Policyهای orgawork_runtime برای جداول P3.2 کامل نیستند.',
  );
  ...
  expectCondition(
    ...
    'Schema drift تاریخی داخل دیتابیس ایزوله P3.2 ظاهر شده است.',
  );
  ...
  expectCondition(
    ...
    'Grant دیتابیسی case.create_self با قرارداد P3.2 سازگار نیست.',
  );

  process.stdout.write('P3.2_REAL_SCHEMA_RLS_PRIVILEGES: VERIFIED\n');
}
...
async function inspectTenantIsolation(
  ...
  await withRuntimeTransaction(access, async (transaction) => {
    ...
    expectCondition(
      ...
      'پرونده‌های P3 بدون Tenant Context قابل مشاهده هستند.',
    );
  });
  ...
  process.stdout.write('P3.2_REAL_TENANT_ISOLATION: VERIFIED\n');
}
...
async function inspectInvariantRejections(
  ...
  process.stdout.write('P3.2_REAL_DATABASE_INVARIANTS: VERIFIED\n');
}
...
const control = createPostgreSqlAccess(
  ...
  {
    applicationName: 'orgawork-p3.2-real-control',
...
const temporaryDatabaseName = `orgawork_p3_acceptance_${randomUUID().replaceAll('-', '')}`;
...
try {
  ...
  process.stdout.write('P3.2_REAL_TEMP_DATABASE: CREATED\n');
  ...
  temporaryAccess = createPostgreSqlAccess(
    ...
    {
      applicationName: 'orgawork-p3.2-real-acceptance',
  ...
  expectCondition(
    migrations.length === 10 &&
      ...
      migrations.at(-1)?.fileName === '0010_create-p3-case-foundation.sql',
    'مجموعه Migration آزمون P3.2 دقیقاً 0001..0010 canonical نیست.',
  );
  ...
  expectCondition(
    ...
    'اجرای دوباره Migrationهای P3.2 idempotent نیست.',
  );

  process.stdout.write('P3.2_REAL_MIGRATIONS_0001_0010: VERIFIED\n');
  ...
  process.stdout.write('P3.2_REAL_RUNTIME_ROLE: VERIFIED\n');
  ...
  process.stdout.write('P3.2_REAL_CREATE_OWN_CASE_FIXTURES: VERIFIED\n');
  ...
  process.stdout.write('P3.2_REAL_POSTGRESQL_ACCEPTANCE: PASSED\n');
} finally {
  ...
  if (databaseCreated) {
    ...
    process.stdout.write('P3.2_REAL_TEMP_DATABASE: DROPPED\n');
  }
