...
const migration = readFileSync('infra/migrations/0010_create-p3-case-foundation.sql', 'utf8');

describe('P3 case foundation PostgreSQL schema', () => {
  ...
  it('enforces one active primary responsibility, action and current work', () => {
    ...
    expect(migration).toContain('orgawork_validate_p3_case_invariants');
  });

  it('keeps P3 current work limited to action or responsibility acceptance', () => {
  ...
  it('uses FORCE RLS and the canonical runtime tenant context on every P3 table', () => {
    for (const table of [
      'orgawork_cases',
      'orgawork_case_responsibilities',
      'orgawork_actions',
